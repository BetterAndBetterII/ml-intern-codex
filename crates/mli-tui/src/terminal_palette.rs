...
///
/// This allows cached renderers to know when their styling assumptions (for example background
/// colors baked into cached transcript rows) may be stale and should be invalidated.
#[allow(dead_code)]
pub fn palette_version() -> u64 {
    DEFAULT_PALETTE_VERSION.load(Ordering::Relaxed)
}

// Upstream CodexPotter uses a forked crossterm with OSC 10/11 color queries
// to auto-detect terminal foreground/background. Our crossterm 0.29.0 doesn't
// expose those APIs, so this fallback returns `None` and the rest of the TUI
// gracefully falls through to default terminal colors.
mod imp {
